...
vkDrawImpl(VkContext* pCtx)
{
    ...
    vkImageTransition(
	    pCmd->handle,
	    drawImage.image.handle,
	    VK_IMAGE_LAYOUT_GENERAL,			/* NOTE: Current layout */
	    VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);	/* NOTE: New layout */
    ...
    vkImageTransition(pCmd->handle, drawImage.image.handle, currentLayout, newLayout);
    vkImageTransition(pCmd->handle, swapchainImage, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
...
vkClearBackground(VkContext* pCtx, VulkanCommandBuffer* pCmd, VkImage pImage)
{
    ...
    f32			flash		= fabs(sin(pCtx->nbFrames / flashSpeed));
